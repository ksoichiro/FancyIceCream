...
public class IceCreamStandRenderer<T extends IceCreamStand> extends EntityRenderer<T, IceCreamStandRenderState> {
    ...
    public void submit(IceCreamStandRenderState iceCreamStand, PoseStack poseStack, SubmitNodeCollector nodeCollector, CameraRenderState camera) {
        ...
        if (!iceCreamStand.standModel.isEmpty()) {
            poseStack.pushPose();
            poseStack.translate(-0.5D, -0.5D, -0.5D);
            iceCreamStand.standModel.submitWithZOffset(poseStack, nodeCollector, iceCreamStand.lightCoords,
    ...
    protected void renderStandItems(IceCreamStandRenderState iceCreamStand, PoseStack poseStack, SubmitNodeCollector nodeCollector) {
        ...
        for (int i = 0; i < itemstacks.size(); i++) {
            ...
            if (!itemstack.isEmpty() && i < iceCreamStand.itemModels.size()) {
                BlockModelRenderState itemModel = iceCreamStand.itemModels.get(i);
                if (!itemModel.isEmpty()) {
                    poseStack.pushPose();
                    poseStack.translate(translations[i][0] / 16.0D, translations[i][1] / 16.0D, translations[i][2] / 16.0D);
                    poseStack.scale(0.8F, 0.8F, 0.8F);
                    poseStack.translate(-0.5D, -0.5D, -0.5D);
                    itemModel.submitWithZOffset(poseStack, nodeCollector, iceCreamStand.lightCoords,
                        OverlayTexture.NO_OVERLAY, iceCreamStand.outlineColor);
                    poseStack.popPose();
                }
            }
    ...
    protected BlockState getBlockStateForItem(ItemStack itemStack) {
        ...
        if (itemResource != null) {
            var definition = BlockStateDefinitions.STATIC_DEFINITIONS.get(
                Identifier.fromNamespaceAndPath(itemResource.getNamespace(), itemResource.getPath()));
            if (definition != null) {
    ...
    public void extractRenderState(T entity, IceCreamStandRenderState state, float partialTick) {
        ...
        // Populate stand model render state
        var resourceLoc = getModelResourceLoc();
        var standDefinition = BlockStateDefinitions.STATIC_DEFINITIONS.get(
            Identifier.fromNamespaceAndPath(resourceLoc.getNamespace(), resourceLoc.getPath()));
        if (standDefinition != null) {
            BlockState standBlockState = standDefinition.any();
            this.blockModelResolver.update(state.standModel, standBlockState, BLOCK_DISPLAY_CONTEXT);
        } else {
            state.standModel.clear();
        }

        // Populate item model render states
        state.itemModels.clear();
        for (ItemStack itemStack : state.items) {
            BlockModelRenderState itemModel = new BlockModelRenderState();
            if (!itemStack.isEmpty()) {
                BlockState blockState = getBlockStateForItem(itemStack);
                if (blockState != null) {
                    this.blockModelResolver.update(itemModel, blockState, BLOCK_DISPLAY_CONTEXT);
                }
            }
            state.itemModels.add(itemModel);
        }
    }
