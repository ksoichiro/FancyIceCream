...
#
# Usage:
#   ./scripts/build-1.16.5.sh              # clean build -x test
#   ./scripts/build-1.16.5.sh clean        # clean only
#   ./scripts/build-1.16.5.sh [gradle args]  # custom gradle args
...
JAVA17_HOME="$(mise where java temurin-17 2>/dev/null || mise where java "$(mise list java 2>/dev/null | grep temurin-17 | head -1 | awk '{print $1}')" 2>/dev/null)" || {
...
if [ $# -eq 0 ]; then
    ...
    "$GRADLE_CMD" -Ptarget_mc_version=1.16.5 "$@"
fi
