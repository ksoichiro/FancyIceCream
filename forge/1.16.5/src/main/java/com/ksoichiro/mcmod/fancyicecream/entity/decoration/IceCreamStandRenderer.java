...
public class IceCreamStandRenderer<T extends IceCreamStand> extends EntityRenderer<T> {
    ...
    public void render(T iceCreamStand, float entityYaw, float partialTicks, MatrixStack poseStack, IRenderTypeBuffer bufferIn, int packedLightIn) {
        ...
        blockrenderdispatcher.getModelRenderer().renderModel(poseStack.last(), bufferIn.getBuffer(Atlases.solidBlockSheet()), (BlockState)null, modelmanager.getModel(this.getTextureLocation(iceCreamStand)), 1.0F, 1.0F, 1.0F, packedLightIn, OverlayTexture.NO_OVERLAY);
        poseStack.popPose();
        ...
        for (int i = 0; i < itemstacks.size(); i++) {
            ...
            if (!itemstack.isEmpty()) {
                ...
                if (model.equals(modelmanager.getMissingModel())) {
                    ...
                    poseStack.translate(-0.5D, -0.5D, -0.5D);
                    blockrenderdispatcher.getModelRenderer().renderModel(poseStack.last(), bufferIn.getBuffer(Atlases.solidBlockSheet()), (BlockState)null, model, 1.0F, 1.0F, 1.0F, packedLightIn, OverlayTexture.NO_OVERLAY);
                }
                poseStack.popPose();
            }
        }
